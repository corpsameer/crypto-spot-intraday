...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the active simulated trade monitor loop.")
    parser.add_argument("--interval", type=int, default=None, help="Loop interval in seconds")
    parser.add_argument("--limit", type=int, default=None, help="Optional active simulated trade limit per cycle")
    ...
    print(f"Starting active trade monitor loop interval={interval}s limit={args.limit}")
    try:
        while True:
            try:
                _print_cycle(monitor.run_once(limit=args.limit))
            except Exception as exc:
                print(f"active_trade_monitor cycle failed: {exc}", file=sys.stderr)
            time.sleep(interval)
    except KeyboardInterrupt:
        print("Stopping active trade monitor loop")
        return 0
