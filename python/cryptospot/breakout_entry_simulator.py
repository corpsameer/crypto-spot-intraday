...
SERVICE_NAME = "breakout_entry_simulator"
SIMULATION_NOTE = "Simulated breakout trade created from triggered trade plan. No real order placed."
ENTRY_EVENT_MESSAGE = "Breakout entry triggered and simulated trade opened."

logger = get_logger(__name__)


class BreakoutEntrySimulator:
    def __init__(self):
    ...
    def run_once(self, limit: int = None) -> dict:
        ...
        try:
            plans = self._load_triggered_breakout_plans(limit)
            ...
            for plan in plans:
                try:
                    ...
                    logger.exception("Failed to convert breakout trade_plan id=%s", plan.get("id"))
                    summary["errors"].append({"plan_id": plan.get("id"), "error": str(exc)})
                    summary["skipped"] += 1

            status = "warning" if summary["errors"] else "ok"
            message = f"Converted {summary['trades_created']} triggered breakout plans into simulated trades"
            if summary["errors"]:
                message = f"{message}; errors {len(summary['errors'])}"
            self._write_health(status, message, summary)
            return summary
        except Exception as exc:
            logger.exception("Breakout entry simulator failed")
            summary["errors"].append({"fatal": str(exc)})
            self._write_health("error", str(exc), summary)
            raise

    def _load_triggered_breakout_plans(self, limit: int = None) -> list[dict]:
        query = """
            SELECT
                tp.id,
                tp.scan_run_id,
    ...
    def _merged_plan_payload(self, raw_payload: Any, now: datetime, trade_id: int) -> dict:
        ...
        payload["breakout_entry_simulator"] = {
    ...
    def _write_health(self, status: str, message: str, summary: dict) -> None:
        try:
            ...
            logger.exception("Failed to write breakout entry simulator health log")
