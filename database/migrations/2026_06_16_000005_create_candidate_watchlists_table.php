...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('candidate_watchlists', function (Blueprint $table): void {
            ...
            $table->string('coindcx_symbol')->index(); $table->timestamp('detected_at')->index(); $table->string('candidate_type')->default('watchlist')->index(); $table->string('entry_strategy')->nullable();
            ...
            $table->string('status')->default('open')->index(); $table->text('reason')->nullable(); $table->text('rejection_reason')->nullable(); $table->timestamp('expires_at')->nullable(); $table->json('raw_payload')->nullable(); $table->timestamps();
        });
